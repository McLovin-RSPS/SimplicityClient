...
public class DuelArenaDuelOptionsWidget extends CustomWidget implements WidgetStateListener {
	...
	public void init() {
		...
		for (int i = 0; i < rules.length; i++) {
			...
			add(addConfigButton(RSInterface.interfaceCache[rule_text[i]].message, 1039, 1040, 5, 1, 1000 + i), 25, yPosition);
			add(RSInterface.interfaceCache[rule_text[i]], 43, yPosition);
			yPosition += 20;
		}

		add(addConfigButton("No Weapon Switch", 1039, 1040, 5, 1, 10011), 25, yPosition);
		add(addText("No Weapon Switch", 1, 0xEE9021, false), 43, yPosition);
		
		yPosition += 20;
		
		add(addConfigButton("Show Inventories", 1039, 1040, 5, 1, 10012), 25, yPosition);
