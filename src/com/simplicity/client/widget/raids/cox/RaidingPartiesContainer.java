 ...
 *
 */
public class RaidingPartiesContainer extends CustomWidget implements WidgetStringListener {
	...
	/**
	 * Constructs a new {@link RaidingPartiesContainer}.
	 */
	public RaidingPartiesContainer() {
		super(WIDGET_ID);
	}
	...
	public void init() {
		...
		for (int i = 0; i < ROWS; i++) {
    		...
    		rect.hidden = true;
    		add(rect, x, y);
    		RSInterface members = addCenteredText("1", 1, 0xff981f);
    		members.useNewFonts = true;
    		add(members, 18, 1 + y);
    		RSInterface targetSize = addCenteredText("/1", 1, 0x9f9f9f);
    		targetSize.useNewFonts = true;
    		add(targetSize, 18 + 42, 1 + y);
    		ids.add(id);
    		add(addCenteredText("Name", 1, 0xffffff), 18 + 137, 1 + y);
    		
    		RSInterface preferredLevel = addCenteredText("126", 1, 0x9f9f9f);
    		...
    		add(addTimer(25, 14, 0, 0x9f9f9f, "0:00"), 18 + 139 + 94 + 77 + 53, 3 + y);
			y += RECT_HEIGHT + 1;
		}
	}
